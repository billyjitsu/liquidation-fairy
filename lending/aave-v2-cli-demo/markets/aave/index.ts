...
console.log('Api3AggregatorAdaptorWETHAddress: ', config.Api3AggregatorAdaptorWETH);

export function generateReservesConfig() {
  const reservesConfig = {
    AAVE: strategies.AAVE,
    BAT: strategies.BAT,
    BUSD: strategies.BUSD,
    DAI: strategies.DAI,
    ENJ: strategies.ENJ,
    KNC: strategies.KNC,
    LINK: strategies.LINK,
    MANA: strategies.MANA,
    MKR: strategies.MKR,
    REN: strategies.REN,
    SNX: strategies.SNX,
    SUSD: strategies.SUSD,
    TUSD: strategies.TUSD,
    UNI: strategies.UNI,
    USDC: strategies.USDC,
    USDT: strategies.USDT,
    WETH: strategies.WETH,
    YFI: strategies.YFI,
    ZRX: strategies.ZRX,
    xSUSHI: strategies.xSUSHI,
  };

  for (const asset of config.assets) {
    if (!reservesConfig[asset.assetSymbol]) {
      reservesConfig[asset.assetSymbol] = strategies[asset.assetSymbol];
    }
  }

  return reservesConfig;
}

export function generateReserveAssetsCustom() {
  ...
  for (const asset of config.assets) {
    if (!reserveAssets[asset.assetSymbol]) {
      reserveAssets[asset.assetSymbol] = asset.ERC20;
    }
  }
  return reserveAssets;
}

const reservesConfig = generateReservesConfig();
const reserveAssetsCustom = generateReserveAssetsCustom();
...
export const AaveConfig: IAaveConfiguration = {
  ...
  ReserveAssets: {
    ...
    [eEthereumNetwork.sepolia]: reserveAssetsCustom,
    [eEthereumNetwork.custom]: reserveAssetsCustom,
