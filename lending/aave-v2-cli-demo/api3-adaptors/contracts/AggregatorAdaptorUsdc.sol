...
contract AggregatorAdaptorUsdc is Ownable {
    ...
    constructor(address _UsdcUsdProxy, string memory _name) Ownable() {
    ...
    function latestAnswer() external view returns (int256 value) {
        ...
        uint256 _value = uint256(UsdcUsdPrice) / 10**10;
        value = int256(_value);
    }

    function latestTimestamp() external view returns (uint256 timestamp) {
        ( , timestamp) = readDataFeed(UsdcUsdProxy);
    }
    ...
    function changeProxyAddress(address _UsdcUsdProxy) external onlyOwner {
        UsdcUsdProxy = _UsdcUsdProxy;
    }
