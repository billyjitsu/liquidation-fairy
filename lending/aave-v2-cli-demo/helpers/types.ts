...
// Extract asset symbols from the config
type AssetSymbols = (typeof config.assets)[number]['assetSymbol'];
const assetSymbols = config.assets.map((asset: any) => asset.assetSymbol);

// Define the existing keys
type ExistingKeys =
  | 'WETH'
  | 'DAI'
  | 'TUSD'
...
type ExistingKeysWithoutUsd =
  ...
  | 'ENJ'
  | 'xSUSHI';
type IntersectionKeys = Extract<AssetSymbols, ExistingKeysWithoutUsd>;
const existingKeys = [
  'DAI',
  'AAVE',
  'TUSD',
...
const combinedKeys = [...existingKeys, ...assetSymbols];
...
export interface iAssetBase<T> extends Record<ExistingKeys | AssetSymbols, T> {}

export type iAssetsWithoutETH<T> = Omit<iAssetBase<T>, 'ETH'>;

export type iAssetsWithoutUSD<T> = Omit<iAssetBase<T>, 'USD'>;

export type iAavePoolAssets<T> = Record<IntersectionKeys, T>;
...
export const TokenContractId = Object.fromEntries(combinedKeys.map((key) => [key, key])) as {
  [key: string]: string;
};
